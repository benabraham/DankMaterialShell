...
Singleton {
    ...
    // "normal"   - preset is not the last-applied one
    // "selected" - last applied and monitor values still match it
    // "modified" - last applied but some values were changed since
    function presetStatus(preset) {
        if (!preset || preset.id !== lastAppliedPresetId)
            return "normal";
        const tasks = presetTasks(preset);
        if (tasks.length === 0)
            return "normal";
        const matches = tasks.every(t => getFeatureValue(t.device, t.code) === t.value);
        return matches ? "selected" : "modified";
    }
