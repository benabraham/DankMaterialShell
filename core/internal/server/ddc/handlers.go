...
func handleGetFeature(conn *ipc.ConnWriter, req ipc.Request, m *Manager) {
...
func handleSetFeature(conn *ipc.ConnWriter, req ipc.Request, m *Manager) {
	device, err := params.String(req.Params, "device")
	if err != nil {
		models.RespondError(conn, req.ID, err.Error())
		return
...
func handleResetDefaults(conn *ipc.ConnWriter, req ipc.Request, m *Manager) {
	device, err := params.String(req.Params, "device")
	if err != nil {
		models.RespondError(conn, req.ID, err.Error())
		return
