...
// ScanDevices probes for DDC devices without re-probing ones already
// identified in a prior scan. DDC traffic during a wake sequence can
// disturb some monitors' own brightness handling.
func (m *Manager) ScanDevices() error {
	return m.scanDevices(false)
}

// ForceRescan re-probes and re-reads every device, including ones already
// identified. Use for explicit user-triggered rescans or to refresh a
// device's current values (e.g. after a reset).
func (m *Manager) ForceRescan() error {
...
func (m *Manager) scanDevices(force bool) error {
	...
	for i := 0; i < 32; i++ {
		...
		if !force {
			if cached, ok := m.capCache.Load(deviceID); ok {
...
// probeVCPCode reads a VCP feature during a scan. Brightness gets retries
// because some monitors ignore the first read right after being probed.
func (m *Manager) probeVCPCode(bus int, code byte) (*ddci2c.VCPReply, error) {
	attempts := 1
	if code == VCPBrightness {
		attempts = 3
	}

	var reply *ddci2c.VCPReply
	var err error
	for i := 0; i < attempts; i++ {
		reply, err = m.busManager.GetVCPFeature(bus, ddci2c.DDCCI_ADDR, code)
		if err == nil {
			return reply, nil
		}
		if i < attempts-1 {
...
func (m *Manager) probeFeatures(bus int, caps *ddci2c.ParsedCapabilities) []FeatureInfo {
	...
	for _, codeDef := range KnownVCPCodes {
		reply, err := m.probeVCPCode(bus, codeDef.Code)
		if err != nil {
...
func (m *Manager) ResetDefaults(deviceID string, resetType string) error {
	...
	go func() {
		...
		m.ForceRescan() //nolint:errcheck // fire-and-forget rescan after reset
	}()
...
func (m *Manager) setFeatureDebounced(dev *DeviceCapabilities, code byte, value int) error {
	...
	if existing, exists := m.debounceTimers[key]; exists {
		if existing.Stop() {
			m.debounceWg.Done()
		}
	}
	...
	m.debounceTimers[key] = time.AfterFunc(200*time.Millisecond, func() {
		...
		m.debounceMutex.Lock()
		pending, hasPending := m.debouncePending[key]
...
func (m *Manager) WaitPending() {
	...
	case <-time.After(5 * time.Second):
		log.Debugf("WaitPending timed out waiting for DDC writes")
	}
}
